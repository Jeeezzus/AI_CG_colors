...
if __name__ == "__main__":
    ...
    model = UNet().to(device=device, dtype=torch.bfloat16)
    ...
    def cbk() -> None:
        to_array = lambda x: x.cpu().to(torch.float32).numpy().astype(np.uint8)
        Image.fromarray(to_array(post_lineart(test_lineart[0])[..., 0])).save("lineart.png")
        Image.fromarray(to_array(post_illustration(model(test_lineart)[0]))).save("generation.png")
